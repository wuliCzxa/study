...
void display_cc(uchar hex, bit dot_on) {
    uchar decimal = (hex / 10) % 10;
    uchar unit = hex % 10;

    P1 |= 0x03;
    P0 = smgduan_cc[decimal] | (dot_on ? 0x80 : 0x00);
    P1 &= ~0x01;
    delay_ms(2);
    P1 |= 0x01;

    P1 |= 0x03;
    P0 = smgduan_cc[unit] | (dot_on ? 0x80 : 0x00);
    P1 &= ~0x02;
    delay_ms(2);
    P1 |= 0x02;
}
...
void display_ca(uchar hex, bit dot_on) {
    ...
    P1 &= 0xF3;
    P0 = smgduan_ca[decimal] & (dot_on ? ~0x80 : 0xFF);
    P1 |= 0x04;
    delay_ms(2);
    P1 &= ~0x04;

    P1 &= 0xF3;
    P0 = smgduan_ca[unit] & (dot_on ? ~0x80 : 0xFF);
    P1 |= 0x08;
    delay_ms(2);
    P1 &= ~0x08;
}
